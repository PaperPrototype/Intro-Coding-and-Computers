The below is not a finished lecture (very unfinished) and is still a work in progress 
...
Pyxel gives us code for checking if someone pressed a button. `btnp` function stands for "button pressed". It gives us back a True or False if the person just now some key.
...
We use the variables instead of putting in the actual positions.

The positions for the rect are in a grid. X left and right, or horizontal (horizontal like the horizon is), and Y goes up and down (like your legs go up and down :P).
