...
The `pyxel.run` will make the `update` function happen again, again, and again, preventing our game from stopping. Without `pyxel.run` we end up just making a window (using `pyxel.init`) and then immediately quit.
...
Pyxel gives us code for checking if someone pressed a button. The `btnp` function stands for "button pressed". It gives us back a True or False if the person clicked the key *just now* (AKA, holding down the key will only count as a single click the moment you clicked it).
